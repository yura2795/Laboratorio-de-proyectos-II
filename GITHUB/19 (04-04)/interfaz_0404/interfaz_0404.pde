...
int estado = 1, lastx = 60, lasty = 260, posx = 60, posy, increm = 40, i=0, j=0, k=0;
...
void draw(){
  ...
  ellipse(800,940,10,10);
   ...
   if(D2==0x20)
   {
    ...
    if(buf3.size()>=5 || buf2.size()>=5 || buf.size()>=5)
    {
       if(posx3 < 980 || posx2 < 980 || posx < 980)
       {
          // Channel 1
          posy = buf.remove(0);
          stroke(azul);
...
int decode(byte con1, byte con2, int offset_a){
  ...
  code = (int)map(code,0,100,290,200);
...
int decode2(byte con1, byte con2, int offset_a){
  ...
  code = (int)map(code,0,1550,450,200);
...
int decode3(byte con1, byte con2, int offset_a){
  ...
  code = (int)map(code,0,2500,870,200);
